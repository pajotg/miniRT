...
static t_shared_pt* create_shared_ptr_from_material(t_material *material)
{
	...
	*material_ptr = *material;
	t_shared_pt* ptr = shared_pt_new(material_ptr, (t_free_func)material_free);
	if (!ptr)
...
static t_shared_pt *read_mix(const char* str, int* current)
{
	...
	t_shared_pt *a = read_material(str, current);
	if (!a)
	{
	...
	skip_whitespace(str, current);
	t_shared_pt *b = read_material(str, current);
	if (!b)
	{
...
static t_shared_pt *read_checkerboard(const char* str, int* current)
{
	...
	skip_whitespace(str, current);
	t_shared_pt *base_material = read_material(str, current);
	if (!base_material)
	{
