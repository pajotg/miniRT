...
static void	handle_conversion_v(t_format *format, t_print_state *state)
{
	...
	vec3 = va_arg(state->lst, t_vec3 *);
	state->printed += fprintf(stderr, "%.2f %.2f %.2f", vec3->x, vec3->y,
			vec3->z);
	(void)format;
}
...
static void	handle_conversion_q(t_format *format, t_print_state *state)
{
	...
	quat = va_arg(state->lst, t_quaternion *);
	state->printed += fprintf(stderr, "%.2f %.2f %.2f %.2f", quat->r, quat->i,
			quat->j, quat->k);
	(void)format;
}
