...
/*   Updated: 2021/01/30 20:11:13 by jsimonis      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static void gather_bounces(const t_scene* scene, const t_material_trace_data* trace_data, t_color_hdr* o_hdr)
{
	*o_hdr = (t_color_hdr) { 0, 0, 0 };
	...
	for (int i = 0; i < NUM_SAMPLES_PER_RAY; i++)
	{
		...
		float strength = vec3_dot(&ray.direction, &trace_data->hit->normal);
		...
		o_hdr->r += out.r * strength;
		o_hdr->g += out.g * strength;
		o_hdr->b += out.b * strength;
	}
