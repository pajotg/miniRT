...
static void material_checkerboard(const t_scene* scene, const void* material_data, const t_material_trace_data* trace_data, t_color_hdr* o_hdr)
{
	const t_material_checkerboard* checkerboard = material_data;

	t_material* mat = checkerboard->mat->ptr;
	mat->material_func(scene, mat->material_data, trace_data, o_hdr);
	int checker = 0;
	checker += get_checker(trace_data->hit->location.x * checkerboard->i_checker_size);
	checker += get_checker(trace_data->hit->location.y * checkerboard->i_checker_size);
	checker += get_checker(trace_data->hit->location.z * checkerboard->i_checker_size);
	if (checker & 1)
	{
